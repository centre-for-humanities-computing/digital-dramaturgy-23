...
annotated <- read_csv2(here("gender-and-mask-annotated.csv"))
...
auto_decapitalized %>% 
  mutate(in_auto = TRUE) %>% 
  full_join(annotated, by = c("speaker","play")) %>% 
  mutate(
    in_auto = !is.na(in_auto.x),
    in_manual = !is.na(in_manual)
  ) %>% 
  select(speaker, play, in_auto, in_manual, everything(), -in_auto.x, -in_auto.y) -> gender_and_mask
