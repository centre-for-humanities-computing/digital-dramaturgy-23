...
```{r}
library(grid) # for panel.spacing
...
my_summary %>% 
  ggplot(aes(y = speaker, x = scene_index)) +
    geom_point(show.legend = FALSE) + #aes(col=speaker, size=words), 
    labs(
      title = paste(my_title, my_year, my_file),
...
my_summary |> 
  ggplot() +
    geom_histogram(aes(x=words))
```

```{r}
my_summary |>
  ungroup() |> 
  mutate(sa = str_c(act_number,",", scene_number)) |>
  select(sa) |> 
  distinct(sa)  |> pull(sa) -> scene_act_numbers
scene_act_numbers
```
...
my_summary |>
  ...
  pull(scene_number) -> scene_numbers
scene_numbers
```
...
my_summary |>
  ungroup() |> 
  mutate(sa = str_c(act_number,",", scene_number)) |>
  select(sa, scene_number) |> 
  distinct(sa,scene_number) |> 
  pull(scene_number) -> scene_numbers
```
...
```{r}
my_summary %>% 
  mutate(act_number = factor(act_number)) |> 
  mutate(scene_index = factor(scene_index)) |> 
  #filter(act_number == 1) |> 
  ggplot(aes(y = speaker, x = scene_index, size=words, colour = act_number, alpha = 0.5)) +
    geom_point(show.legend = TRUE) +
    #geom_vline(xintercept = 10, colour = "red") +
    #scale_size_continuous(breaks=c(50,100,150,200,250,300,400,500), range = c(0,20)) +
  scale_size_continuous(range = c(0,20)) +
    
    #scale_x_discrete(breaks = c(1,10,20,30,40,50), labels = c("Act 1", "Act 2", "Act 3", "Act 4", "Act 5", "Act 6")) +
    scale_x_discrete(labels = scene_numbers) +
  #labels=c("0" = "Dose 0.5", "1" = "Dose 1","2" = "Dose 2")
    labs(
      title = paste(my_title, my_year, my_file),
      subtitle = "Hvem taler hvornår"
    ) +
...
```{r}
ggsave("~/Desktop/bobler.pdf", width=12, height=8)
```
